...
pushd "$root" > /dev/null
  ...
  pushd src/lib/crypto > /dev/null
    marlin_commit_id="${PROOF_SYSTEMS_REPO_SHA-$(git rev-parse --verify HEAD || echo "<unknown>")}"
    ...
    marlin_commit_date="${PROOF_SYSTEMS_REPO_SHA-$(git show HEAD -s --format="%cI" || echo "<unknown>")}"
  popd > /dev/null
popd > /dev/null
